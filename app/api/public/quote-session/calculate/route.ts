...
export async function POST(request: NextRequest) {
  try {
    ...
    type StyleColour = { fence_style_id: string; color_name: string | null };
    let styleColour: StyleColour | null = null;
    let allowColourPricingFallback = false;
    ...
    if (colour_option_id) {
      ...
      if (colour?.fence_style_id) {
        styleColour = { fence_style_id: colour.fence_style_id, color_name: colour.color_name ?? null };
        allowColourPricingFallback = true;
      }
    } else if (fence_style_id) {
      styleColour = { fence_style_id: fence_style_id, color_name: null };
      allowColourPricingFallback = false;
    }

    if (styleColour) {
      const colour = styleColour;
      ...
      if (!ruleRow && allowColourPricingFallback && colour_option_id) {
    ...
    const hasHierarchySelection = !!(colour_option_id || fence_style_id);
    if (!ruleRow || (!hasHierarchySelection && !product_option_id)) {
      return NextResponse.json(
        {
          error:
            colour_option_id || product_option_id || fence_style_id
              ? 'Pricing not found for this option'
              : 'Missing product_option_id, colour_option_id, or fence_style_id',
        },
        { status: 400 }
      );
    }

    const optionId = String(colour_option_id || fence_style_id || product_option_id);
