...
export type AuthLinkPasswordFormCopy = {
  title: string;
  titleNoSession: string;
  subtitleNoSession: string;
  subtitleWithSession: string;
  submitLabel: string;
  submitLabelLoading: string;
  defaultLinkError: string;
  backToLogin: string;
};

type Props = {
  copy: AuthLinkPasswordFormCopy;
  onPasswordSaved: () => void;
};

export function AuthLinkPasswordForm({ copy, onPasswordSaved }: Props) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- one-time URL → session handoff
  }, []);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-xl">
          <h1 className="text-2xl font-bold tracking-tight">{copy.titleNoSession}</h1>
          <p className="mt-2 text-sm text-slate-600">
            {linkError ? linkError : copy.subtitleNoSession}
          </p>
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-xl">
        ...
        <p className="mt-2 text-sm text-slate-600">{copy.subtitleWithSession}</p>
        <form onSubmit={submit} className="mt-6 space-y-3">
          ...
          >
            {saving ? copy.submitLabelLoading : copy.submitLabel}
          </button>
        </form>
      </div>
    </div>
